...
public class ServerService implements IServerService {
        ...
        return subjectRepository.findById(subjectId)
                .map(subject -> subject.getEnrolledStudyIds().stream()
                        .mapToInt(studyIdWithYear -> {
                            ...
                            return getAllStudentsByStudyIdAndAttendanceYear(parts[0], Integer.parseInt(parts[1])).size();
                        })
                        .sum()
                )
                .orElse(-1);
    ...
    @Override
    public List<StudentDTO> getAllStudentsByStudyIdAndAttendanceYear(String studyId, int attendanceYear) {
    ...
    public void startNewClassInstance(boolean isExercise, String subjectId, String begin, String end) {
        ...
        Date beginsAtUTC = Date.from(LocalDate.now()
                .atTime(LocalTime.parse(begin, DateTimeFormatter.ofPattern("HH:mm")))
                .atZone(ZoneId.systemDefault())
                .toInstant());
        Date endsAtUTC = Date.from(LocalDate.now()
                .atTime(LocalTime.parse(end, DateTimeFormatter.ofPattern("HH:mm")))
                .atZone(ZoneId.systemDefault())
                .toInstant());
    ...
    public List<StaffDTO> getAllAssistants() {
        return staffMapper.toDTOList(staffRepository.findAllByRole("assistant"));
    }
