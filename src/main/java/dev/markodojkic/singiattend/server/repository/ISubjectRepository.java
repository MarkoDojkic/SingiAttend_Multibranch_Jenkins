...
public interface ISubjectRepository extends MongoRepository<Subject, String> {
    ...
    AggregationResults<AttendanceHelperInstance> getAttendanceHelperInstanceById(String id);
    ...
    @Aggregation(pipeline={"{ $match: { 'professor_id': {$eq: ?0} } }",
            "{ $lookup: {from:'Staff',localField:'professor_id',foreignField:'_id',as:'professor'} }",
            "{$lookup: {from:'Staff',localField:'assistant_id',foreignField:'_id',as:'assistant'} }"})
    AggregationResults<Subject> findAllAggregationByProfessorId(String professorId);
    @Aggregation(pipeline={"{ $match: { 'assistant_id': {$eq: ?0} } }",
            "{ $lookup: {from:'Staff',localField:'professor_id',foreignField:'_id',as:'professor'} }",
            "{$lookup: {from:'Staff',localField:'assistant_id',foreignField:'_id',as:'assistant'} }"})
    AggregationResults<Subject> findAllAggregationByAssistantId(String assistantId);
    boolean existsByProfessorIdOrAssistantId(String professorId, String assistantId);
}
